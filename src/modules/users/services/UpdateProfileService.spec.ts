...
describe('UpdateProfile', () => {
  ...
  it('should be able update profile', async () => {
    ...
    const updateUser = await updateProfile.execute({
      user_id: user.id,
      name: 'Joel Miller',
      email: 'joelmillher@exemple.com',
    });

    expect(updateUser.name).toBe('Joel Miller');
    expect(updateUser.email).toBe('joelmillher@exemple.com');
  });
  ...
  it('should not be to change the email to a existing email', async () => {
  ...
  it('should not be able update password with wrong old password', async () => {
    ...
    await expect(
      updateProfile.execute({
        ...
        name: 'Joel Miller',
        email: 'joelmillher@exemple.com',
        old_password: 'wrong-old-passwors',
        password: '123123',
      }),
    ).rejects.toBeInstanceOf(AppError);
  });
